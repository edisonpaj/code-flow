...
async function loadEndpoints(){ try { resetProjectAnalysis();const project=projects[+$('project').value]; if(!project)return;status(`Carregando ${project.name}…`);const [data,swagger,profile]=await Promise.all([json('/api/endpoints?project='+encodeURIComponent(project.path)),json('/api/swagger-info?project='+encodeURIComponent(project.path)),json('/api/project-profile?project='+encodeURIComponent(project.path))]);swaggerInfo=swagger;$('endpoint').innerHTML=data.endpoints.length?data.endpoints.map(e=>`<option value="${e.id}">${e.http_method} ${e.path}</option>`).join(''):'<option>Nenhum endpoint</option>';renderArchitecture(profile.architecture);$('swagger').disabled=!swagger.detected;$('swagger').title=swagger.detected?`Abrir ${swagger.swagger_url}`:'Springdoc/Swagger não identificado no build';status(`${project.name}: ${data.endpoints.length} endpoint(s). Arquitetura ${profile.architecture.label}.${swagger.detected?' Swagger: '+swagger.swagger_url:' Swagger não identificado.'}`);rememberSelectedProject(); }catch(e){swaggerInfo=null;$('swagger').disabled=true;$('architecture').textContent='Não foi possível detectar a arquitetura.';status(e.message,true);} }
...
$('load').onclick=async()=>{ $('capacity').disabled=true;$('xray').disabled=true;await originalLoadProjects();$('capacity').disabled=!projects.length;$('xray').disabled=!projects.length;rememberSelectedProject(); };
