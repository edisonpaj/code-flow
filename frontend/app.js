...
const themeKey='expert-code-flow:theme';
const activeRequests=new Map();
let requestSequence=0;
function updateRequestControls(){const cancel=$('cancel-load');if(cancel)cancel.hidden=!activeRequests.size;}
function cancelRequest(key,reason='cancelled'){const active=activeRequests.get(key);if(!active)return false;active.cancelled=true;active.controller.abort(reason);if(active.cancelUrl)fetch(active.cancelUrl,{method:'POST',keepalive:true}).catch(()=>{});activeRequests.delete(key);updateRequestControls();return true;}
function cancelActiveRequests(message='Operação cancelada.'){const keys=[...activeRequests.keys()];keys.forEach(key=>cancelRequest(key));if(keys.length&&message)status(message);return keys.length;}
async function requestJson(url,{key=`request-${++requestSequence}`,timeoutMs=30000,cancelUrl=null,fetchOptions={}}={}){
  cancelRequest(key,'superseded');
  const controller=new AbortController(),active={controller,cancelUrl,cancelled:false,timedOut:false};
  const timer=setTimeout(()=>{active.timedOut=true;controller.abort('timeout');if(cancelUrl)fetch(cancelUrl,{method:'POST',keepalive:true}).catch(()=>{});},timeoutMs);
  activeRequests.set(key,active);updateRequestControls();
  try{
    const headers={...fetchOptions.headers,'X-Request-ID':key};
    const response=await fetch(url,{...fetchOptions,headers,signal:controller.signal});
    const text=await response.text();let body={};try{body=text?JSON.parse(text):{};}catch{body={detail:text||'Resposta inválida do servidor'};}
    if(!response.ok)throw new Error(body.detail||`Falha na requisição (${response.status})`);
    return body;
  }catch(error){
    if(error.name==='AbortError'||controller.signal.aborted){const cancelled=new Error(active.timedOut?`A operação excedeu ${Math.round(timeoutMs/1000)} segundos e foi encerrada.`:'Operação cancelada pelo usuário.');cancelled.cancelled=true;throw cancelled;}
    throw error;
  }finally{
    clearTimeout(timer);if(activeRequests.get(key)===active)activeRequests.delete(key);updateRequestControls();
  }
}
window.addEventListener('pagehide',()=>cancelActiveRequests(''));
document.querySelectorAll('.sidebar a,.concept-card').forEach(link=>link.addEventListener('click',()=>cancelActiveRequests('')));
...
const processingState={logs:[],poller:null,phase:'idle'};
function setTheme(theme){const light=theme==='light';document.body.classList.toggle('light-theme',light);const button=$('theme-toggle');if(button){button.textContent=light?'Tema escuro':'Tema claro';button.setAttribute('aria-pressed',String(light));}localStorage.setItem(themeKey,theme);}
function initTheme(){let saved='dark';try{saved=localStorage.getItem(themeKey)||'dark';}catch{}setTheme(saved==='light'?'light':'dark');const button=$('theme-toggle');if(button)button.onclick=()=>setTheme(document.body.classList.contains('light-theme')?'dark':'light');}
function addProcessingLog(message,level='info'){const time=new Date().toLocaleTimeString('pt-BR');processingState.logs.unshift({time,message,level});processingState.logs=processingState.logs.slice(0,40);renderProcessingLog();}
function renderProcessingLog(){const target=$('processing-log');if(!target)return;target.innerHTML=processingState.logs.length?processingState.logs.map(item=>`<p class="${item.level==='error'?'error':item.level==='muted'?'muted':''}"><span>${escapeXml(item.time)}</span> ${escapeXml(item.message)}</p>`).join(''):'<p>Nenhuma operação iniciada nesta tela.</p>';}
function setProcessing(step,percent,message,level='info'){const title=$('processing-title'),bar=$('processing-bar'),label=$('processing-percent');if(title)title.textContent=message;if(bar)bar.style.width=`${Math.max(0,Math.min(100,percent))}%`;if(label)label.textContent=`${Math.round(Math.max(0,Math.min(100,percent)))}%`;document.querySelectorAll('.processing-steps span').forEach(item=>{const order=['directory','projects','endpoints','ready'];const current=order.indexOf(step),index=order.indexOf(item.dataset.step);item.classList.toggle('done',index>=0&&current>=0&&index<current);item.classList.toggle('active',item.dataset.step===step);});if(message)addProcessingLog(message,level);}
function stopProcessingStatusPoll(){if(processingState.poller){clearInterval(processingState.poller);processingState.poller=null;}}
async function refreshProcessingStatus(){try{const data=await requestJson('/api/status',{key:'status-monitor',timeoutMs:5000});const last=data.last_processing;if(last){const suffix=last.status==='running'?`worker ativo pid ${last.pid||'-'}`:`${last.status} em ${last.duration_ms||0} ms`;addProcessingLog(`Backend: ${suffix}`,'muted');}const latest=(data.processing_events||[])[0];if(latest?.message)addProcessingLog(`API: ${latest.message}`,'muted');}catch(error){addProcessingLog(`Status indisponível: ${error.message}`,'error');}}
function startProcessingStatusPoll(){stopProcessingStatusPoll();refreshProcessingStatus();processingState.poller=setInterval(refreshProcessingStatus,5000);}

$('cancel-load').onclick=()=>cancelActiveRequests('Carregamento cancelado. Você pode navegar ou iniciar outra análise.');
...
async function json(url,options={}){return requestJson(url,options);}
$('load').onclick = async () => { cancelActiveRequests('');stopProcessingStatusPoll();processingState.logs=[];const rootValue=$('root').value.trim();setProcessing('directory',12,`Diretório definido: ${rootValue||'vazio'}`);try { resetProjectAnalysis();$('project').innerHTML='<option>Carregando projetos…</option>';$('endpoint').innerHTML='<option>Aguardando projeto…</option>';status('Procurando projetos Spring Boot…');setProcessing('projects',32,'Procurando microsserviços Spring Boot no diretório.'); const data = await json('/api/projects?root='+encodeURIComponent(rootValue),{key:'project-discovery',timeoutMs:30000}); projects=data.projects; $('project').innerHTML=projects.length?projects.map((p,i)=>`<option value="${i}">${escapeXml(p.name)}</option>`).join(''):'<option>Nenhum projeto Spring Boot</option>'; status(`${projects.length} projeto(s) encontrado(s).`);setProcessing('projects',52,`${projects.length} projeto(s) Spring Boot encontrado(s).`); if(projects.length) await loadEndpoints();else{$('architecture').textContent='Nenhum microsserviço Spring Boot encontrado.';$('endpoint').innerHTML='<option>Nenhum endpoint</option>';setProcessing('ready',100,'Nenhum microsserviço Spring Boot encontrado.','error');} } catch(e){stopProcessingStatusPoll();if(e.cancelled){status(e.message);setProcessing('ready',100,e.message,'error');return;}projects=[];$('project').innerHTML='<option>Diretório inválido</option>';$('endpoint').innerHTML='<option>Nenhum endpoint</option>';$('architecture').textContent='Não foi possível detectar a arquitetura.';status(e.message,true);setProcessing('ready',100,`Falha ao carregar diretório: ${e.message}`,'error');} };
async function loadEndpoints(){cancelRequest('endpoint-scan');stopProcessingStatusPoll();try { resetProjectAnalysis();const project=projects[+$('project').value]; if(!project)return;const analysisId=`scan-${Date.now()}-${Math.random().toString(16).slice(2)}`,cancelUrl='/api/analysis/cancel?analysis_id='+encodeURIComponent(analysisId);status(`Analisando endpoints de ${project.name}… Você pode cancelar ou navegar para outra tela.`);setProcessing('endpoints',68,`Analisando endpoints de ${project.name}. ID ${analysisId}.`);startProcessingStatusPoll();const data=await json('/api/endpoints?project='+encodeURIComponent(project.path)+'&analysis_id='+encodeURIComponent(analysisId),{key:'endpoint-scan',timeoutMs:125000,cancelUrl});stopProcessingStatusPoll();swaggerInfo=null;$('endpoint').innerHTML=data.endpoints.length?data.endpoints.map(e=>`<option value="${e.id}">${e.http_method} ${e.path}</option>`).join(''):'<option>Nenhum endpoint</option>';renderArchitecture(data.architecture);$('swagger').disabled=true;$('swagger').title='Swagger temporariamente desativado';status(`${project.name}: ${data.endpoints.length} endpoint(s). Arquitetura ${data.architecture.label}. Swagger temporariamente desativado.`);setProcessing('ready',100,`${data.endpoints.length} endpoint(s) carregado(s). Arquitetura ${data.architecture.label}.`);rememberSelectedProject(); }catch(e){stopProcessingStatusPoll();swaggerInfo=null;$('swagger').disabled=true;if(e.cancelled){status(e.message);setProcessing('ready',100,e.message,'error');return;}$('architecture').textContent='Não foi possível detectar a arquitetura.';status(`${e.message} Consulte logs/api.log.`,true);setProcessing('ready',100,`Falha ao trazer endpoints: ${e.message}`,'error');} }
...
$('root').addEventListener('input',()=>{clearTimeout(rootChangeTimer);lastLoadedRoot='';});
...
initTheme();
setProcessing('directory',0,'Aguardando diretório','muted');
loadAnalysisEngines();
